...
def moe_align_block_size_stage1_api(topk_ids, tokens_cnts, num_experts=-1):

    assert len(topk_ids.shape) == 2
    numel = topk_ids.shape[0] * topk_ids.shape[1]
    grid = (num_experts,)
    tokens_per_thread = ceil_div(numel, num_experts)

    prepare_attr_for_triton_kernel = """
    int numel = topk_ids.shape()[0] * topk_ids.shape()[1];
    int tokens_per_thread = triton_cdiv(numel, num_experts);
    """

    op_name = f"moe_align_block_size_stage1{get_dtype_str(topk_ids.dtype)}"
    op_name += f"_{get_dtype_str(tokens_cnts.dtype)}"
    op_name += f"_{num_experts}"

    if op_name not in OpProtoHolder.instance().op_proto_map.keys():
        prepare_ptr_for_triton_kernel = """
        CUdeviceptr input_ptrs[2] = {
            get_tensor_ptr(topk_ids),
            get_tensor_ptr(tokens_cnts),
        };

        """

        template_used = rendering_common_template(
            moe_align_block_size_stage1_api,
            prepare_attr_for_triton_kernel,
            prepare_ptr_for_triton_kernel,
        )
        grid = (f"{num_experts}",)

        moe_align_block_size_stage1_zkk[(op_name, template_used, grid)](
            topk_ids,
            tokens_cnts,
            -1,
            -1,
    ...
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op(op_name, topk_ids, tokens_cnts, num_experts)
        return outs[0]
    else:
        helper = LayerHelper(op_name, **locals())
        inputs = {
...
def moe_align_block_size_stage2_api(place_holder, tokens_cnts, num_experts=256):
    grid = (num_experts,)

    op_name = f"moe_align_block_size_stage2_{get_dtype_str(tokens_cnts.dtype)}_{num_experts}"
    if op_name not in OpProtoHolder.instance().op_proto_map.keys():
        prepare_ptr_for_triton_kernel = """
        CUdeviceptr input_ptrs[1] = {
            get_tensor_ptr(tokens_cnts),
        };
        """

        template_used = rendering_common_template(
            moe_align_block_size_stage2_api,
            "",
            prepare_ptr_for_triton_kernel,
        )
        grid = (f"{num_experts}",)

        moe_align_block_size_stage2_zkk[(op_name, template_used, grid)](
            tokens_cnts,
            num_experts,
        )
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op(op_name, place_holder, tokens_cnts, num_experts)
        return outs[0]
    else:
        helper = LayerHelper(op_name, **locals())
        inputs = {
...
def moe_align_block_size_stage3_api(
    place_holder, num_tokens_post_pad, tokens_cnts, cumsum, num_experts=-1, block_size=64
):

    op_name = f"moe_align_block_size_stage3_{get_dtype_str(num_tokens_post_pad.dtype)}"
    op_name += f"_{get_dtype_str(tokens_cnts.dtype)}"
    op_name += f"_{get_dtype_str(cumsum.dtype)}"
    op_name += f"_{num_experts}"
    ...
    if op_name not in OpProtoHolder.instance().op_proto_map.keys():
        prepare_ptr_for_triton_kernel = """
        CUdeviceptr input_ptrs[3] = {
            get_tensor_ptr(num_tokens_post_pad),
            get_tensor_ptr(tokens_cnts),
            get_tensor_ptr(cumsum),
        };
        """

        template_used = rendering_common_template(
            moe_align_block_size_stage3_api,
            "",
            prepare_ptr_for_triton_kernel,
        )
        grid = (f"{1}",)

        moe_align_block_size_stage3_zkk[(op_name, template_used, grid)](
            num_tokens_post_pad,
            tokens_cnts,
            cumsum,
            num_experts,
    ...
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op(
            op_name, place_holder, num_tokens_post_pad, tokens_cnts, cumsum, num_experts, block_size
        )
        return outs[0]
    else:
        helper = LayerHelper(op_name, **locals())
        ...
        attrs = {"num_experts": num_experts, "block_size": block_size}
        useless = helper.create_variable_for_type_inference(dtype="int32")
        outputs = {"useless": useless}
        helper.append_op(
            type=op_name,
...
def moe_align_block_size_stage4_api(
    place_holder, topk_ids, sorted_token_ids, expert_ids, tokens_cnts, cumsum, num_experts=-1, block_size=64
):
    numel = topk_ids.shape[0] * topk_ids.shape[1]
    tokens_per_thread = ceil_div(numel, num_experts)

    op_name = f"moe_align_block_size_stage4_{get_dtype_str(topk_ids.dtype)}_{num_experts}"
    op_name += f"_{get_dtype_str(tokens_cnts.dtype)}"
    op_name += f"_{get_dtype_str(tokens_cnts.dtype)}"
    op_name += f"_{block_size}"
    ...
    if op_name not in OpProtoHolder.instance().op_proto_map.keys():
        ...
        template_used = rendering_common_template(
            moe_align_block_size_stage4_api,
            prepare_attr_for_triton_kernel,
            prepare_ptr_for_triton_kernel,
        )
        grid = (f"{num_experts}",)

        moe_align_block_size_stage4_zkk[(op_name, template_used, grid)](
            topk_ids,
            sorted_token_ids,
            expert_ids,
            tokens_cnts,
    ...
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op(
            op_name, place_holder, topk_ids, sorted_token_ids, expert_ids, tokens_cnts, cumsum, num_experts, block_size
        )
        return outs[0]
    else:
        helper = LayerHelper(op_name, **locals())
...
def moe_align_block_size_triton(
    ...
    # topk_ids1 = paddle.assign(topk_ids)
    # sorted_token_ids1 = paddle.assign(sorted_token_ids)
    # expert_ids1 = paddle.assign(expert_ids)
    # num_tokens_post_pad1 = paddle.assign(num_tokens_post_pad)
    # cumsum1 = paddle.assign(cumsum)
    # tokens_cnts1 = paddle.assign(tokens_cnts)

    useless = moe_align_block_size_stage1_api(topk_ids, tokens_cnts, num_experts)
    useless = moe_align_block_size_stage2_api(useless, tokens_cnts, num_experts)
    useless = moe_align_block_size_stage3_api(
        useless, num_tokens_post_pad, tokens_cnts, cumsum, num_experts, block_size
    )
    useless = moe_align_block_size_stage4_api(
        useless, topk_ids, sorted_token_ids, expert_ids, tokens_cnts, cumsum, num_experts, block_size
    )
    return
    ...
    # assert (topk_ids1-topk_ids).abs().max().item() == 0
    # assert (sorted_token_ids1-sorted_token_ids).abs().max().item() == 0
    # assert (expert_ids1-expert_ids).abs().max().item() == 0
    # assert (num_tokens_post_pad1-num_tokens_post_pad).abs().max().item() == 0
    # assert (cumsum1-cumsum).abs().max().item() == 0
    # assert (tokens_cnts1-tokens_cnts).abs().max().item() == 0
...
def invoke_fused_moe_kernel(
    ...
    # assert compute_type == tl.float16
...
def invoke_fused_moe_kernel_api(
    ...
    if op_name not in OpProtoHolder.instance().op_proto_map.keys():
        ...
        fused_moe_kernel_zkk[(op_name, template_used, grid, [config])](
            A,
            B,
            C,
            A_scale,
...
def fused_experts_impl(
    ...
    intermediate_cache1 = intermediate_cache1.reshape([-1, N])
