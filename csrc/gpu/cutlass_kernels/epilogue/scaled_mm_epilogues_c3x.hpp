...
#include "cutlass_kernels/epilogue/broadcast_load_epilogue_c3x.hpp"
...
struct ScaledEpilogueBase {
 protected:
  ...
  template <typename Descriptor, typename T>
  static auto args_from_tensor(std::optional<paddle::Tensor> const& tensor) {
    using Arguments = typename Descriptor::Arguments;
    auto* data_ptr = tensor ? static_cast<T*>(const_cast<void*>(tensor->data())) : nullptr;
    static_assert(std::is_same_v<Descriptor, ColLoad<T, true>> ||
                  std::is_same_v<Descriptor, RowLoad<T, true>>);
    return Arguments{data_ptr};
  }
