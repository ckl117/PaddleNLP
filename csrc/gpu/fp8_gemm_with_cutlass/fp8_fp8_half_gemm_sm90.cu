...
std::vector<paddle::Tensor> cutlass_fp8_fp8_half_gemm_sm90(
  ...
  if (x.dtype() == phi::DataType::FLOAT8_E4M3FN) {
    ...
    PADDLE_THROW(phi::errors::Fatal(
        "fp8_fp8_half_gemm_fused_sm90 only support e4m3 and e5m2 input"));
  }
  ...
  if (x.dtype() == phi::DataType::FLOAT8_E4M3FN){
    if (output_dtype == "float16") {
      dispatch_fuse_gemm_noact_sm90<phi::dtype::float8_e4m3fn, phi::dtype::float16,
  ...
  // else{
  //   if (output_dtype == "bfloat16") {
  //     dispatch_fuse_gemm_noact_3x<phi::dtype::float8_e5m2, phi::dtype::bfloat16>(params);
  //   }else{
  //     dispatch_fuse_gemm_noact_3x<phi::dtype::float8_e5m2, phi::dtype::float16>(params);
  //   }
  // }
...
std::vector<std::vector<int64_t>> CutlassFp8Fp8HalfGemmSm90FusedInferShape(
...
std::vector<paddle::DataType> CutlassFp8Fp8HalfGemmSm90FusedInferDtype(
    ...
    if (output_dtype == "bfloat16")
        ...
        PD_THROW(
                "fp8_fp8_half_gemm_fused_sm90 only support bfloat16 and float16 output");
    return {data_type};
}

PD_BUILD_OP(cutlass_fp8_fp8_half_gemm_sm90_fused)
    ...
    .Attrs({"transpose_x: bool",
            ...
            "act: std::string"})
    .Outputs({"out"})
    .SetKernelFn(PD_KERNEL(cutlass_fp8_fp8_half_gemm_sm90))
    .SetInferShapeFn(PD_INFER_SHAPE(CutlassFp8Fp8HalfGemmSm90FusedInferShape))
