...
code_part7_1 = """
        ...
        if(params.batch_count > 1) {
          throw std::runtime_error("cutlass gemm split_k mode only support batch_count = 1, but current batch_count > 1.");
          break;
        }
...
code_part8 = """
          case {tile_id}:
            return dispatch_dual_gemm_split_k_{act_tag}<phi::dtype::{input_type}, phi::dtype::{output_type}, phi::dtype::{bias_type},
                                  cutlass::gemm::GemmShape{thread_block_shape}, cutlass::gemm::GemmShape{warp_shape},
                                  cutlass::gemm::GemmShape{mma_shape}, {num_stages}, {hasbias}, {SM}>(params);
            break;"""
...
def generate_dispatch_dual_gemm_cu(
                    ...
                    if min_split_k > 1 and max_split_k > 1:
                        ...
                        for tile in tiles:
                            for stage in stages:
                                ...
                                all_code += SubstituteTemplate(code_part8, value_dict)
                                tile_id += 1
                        all_code += code_part9

                    else:
                        all_code += code_part7_0
    ...
    all_code += SubstituteTemplate(code_part10, value_dict)
    return all_code
