...
LaunchGemmPart1 = """
        case {type_id}:
            return dispatch_fuse_gemm_act_sm90<phi::dtype::{input_type}, phi::dtype::{output_type},
                                 {hasbias},
                                 cutlass::epilogue::thread::{Activation},
                                 Shape{TileShape},
                                 Shape{ClusterShape},
...
code_part3 = """    {"{TileShape}, {ClusterShape}, {KernelSchedule}, {EpilogueSchedule}", {tile_id}},
"""
...
bool fp8_fp8_gemm_scale_bias_act_sm{sm}(GemmEpilogueAllParams params) {
  ...
  if(getenv("FLAGS_use_cutlass_device_best_config_path")){ // run kernel
    ...
    return launch_gemm_kernel_sm90(type_id, kernel_id, params);
  } else { // tune kernel
    int warm_up_times = 5;
    int tune_times = 10;
    ...
    for(const auto& config_pair : gemm_config_map){
        ...
        for(int num_time = 0; num_time < warm_up_times; ++num_time){
            if(!launch_gemm_kernel_sm90(type_id, config_pair.second, params)){
                is_valid = false;
                break;
            }
        }
    ...
    for tile_shape in tiles:
        ...
        for kernel_schedule in KernelSchedule:
            ...
            for epilogue_schedule in EpilogueSchedule:
                ...
                source_path = os.path.join(generate_dir, f"launch_gemm_kernel_sm{sm[-2:]}_{gemm_config_str}.cu")
    ...
    for sm in archs:
        if sm == "90":
            ...
            for fuse_gemm_config in fuse_gemm_configs:
                file_name = f"gpu/cutlass_kernels/fp8_gemm_fused/autogen/generic_gemm_kernel_sm{sm}_{fuse_gemm_config[1][0]}.cu"
                all_code = generate_source_cu(
                    inputs_type,
                    outputs_type,
                    fuse_gemm_config[0],
                    fuse_gemm_config[1][1],
                    fuse_gemm_config[2],
                    fuse_gemm_config[3],
                    fuse_gemm_config[4],
                    sm_dict[sm],
            ...
            file_name = f"gpu/cutlass_kernels/fp8_gemm_fused/autogen/fp8_fp8_gemm_scale_bias_act_sm{sm}.cu"
