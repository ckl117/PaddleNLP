...
def get_candidate_configs(sm):
    ...
    hasbias = ("false", "true")
    KernelSchedule = ("KernelTmaWarpSpecializedCooperativeFP8BlockScaledAccum<1>",)
...
GemmDeclare = """
template<>
bool dispatch_fuse_gemm_per_block<phi::dtype::{input_type}, phi::dtype::{output_type},
                                 {hasbias},
...
LaunchGemmDeclare = """
bool launch_per_block_gemm_kernel_sm{sm}_{gemm_config}(const int type_id, GemmEpilogueAllParams params);
"""

LaunchGemmPart0 = """
#pragma once

#include "launch_per_block_gemm_kernel_sm{sm}.h"
...
LaunchGemmPart1 = """
        case {type_id}:
            return dispatch_fuse_gemm_per_block<phi::dtype::{input_type}, phi::dtype::{output_type},
                                 {hasbias},
                                 cutlass::epilogue::thread::{Activation},
                                 Shape{TileShape},
                                 Shape{ClusterShape},
...
std::map<std::string, int> per_block_gemm_type_map{"""
...
std::map<std::string, int> per_block_gemm_config_map{
"""
...
bool launch_per_block_gemm_kernel(const int type_id, const int kernel_id, GemmEpilogueAllParams params){
    switch (kernel_id) {"""

code_part5 = """
        case {tile_id}:
            return launch_per_block_gemm_kernel_sm{sm}_{gemm_config}(type_id, params);
            break;
"""
...
T get_relative_best(nlohmann::json* json_data,
    ...
    if (json_data->contains(target_key)) {
        ...
        std::cout << "Warning: use default config" << std::endl;
        return "<_128, _128, _128>, <_1, _1, _1>, KernelTmaWarpSpecializedCooperativeFP8BlockScaledAccum<1>, TmaWarpSpecializedCooperative";
    }
}

bool fp8_per_block_gemm_scale_bias_act(GemmEpilogueAllParams params) {
  if (per_block_gemm_type_map.find(params.fuse_gemm_config) == per_block_gemm_type_map.end()) {
    throw std::runtime_error("fp8 gemm_fused config is invalid.");
  }

  int type_id = per_block_gemm_type_map[params.fuse_gemm_config];
  ...
  if(config_file_path == "tune"){ // tune kernel
    ...
    for(const auto& config_pair : per_block_gemm_config_map){
        std::cout << "Running tune kernel: " << config_pair.first<< std::endl;
        bool is_valid = true;
        // warm up
        for(int num_time = 0; num_time < warm_up_times; ++num_time){
            if(!launch_per_block_gemm_kernel(type_id, config_pair.second, params)){
                is_valid = false;
                break;
            }
        }
    ...
    if (per_block_gemm_config_map.find(best_config) == per_block_gemm_config_map.end()) {
        ...
        kernel_id = per_block_gemm_config_map[best_config];
    }
    return launch_per_block_gemm_kernel(type_id, kernel_id, params);
  }
}
...
def generate_launch_gemm_cus(
    ...
    code_map = {}
    head_path = os.path.join(generate_dir, f"launch_per_block_gemm_kernel_sm{sm[-2:]}.h")
        ...
        for kernel_schedule in KernelSchedule:
            ...
            for epilogue_schedule in EpilogueSchedule:
                ...
                source_path = os.path.join(
                    generate_dir, f"launch_per_block_gemm_kernel_sm{sm[-2:]}_{gemm_config_str}.cu"
                )
                print(source_path)
    ...
    for sm in archs:
        if sm == "90":
            ...
            for fuse_gemm_config in fuse_gemm_configs:
                file_name = f"gpu/cutlass_kernels/fp8_gemm_fused/autogen/generic_per_block_gemm_kernel_sm{sm}_{fuse_gemm_config[1][0]}.cu"
            ...
            file_name = f"gpu/cutlass_kernels/fp8_gemm_fused/autogen/fp8_per_block_gemm_scale_bias_act_sm{sm}.cu"
